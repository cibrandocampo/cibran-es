...
argument-hint: <area to audit, e.g.: "Tailwind design tokens", "portfolio section", "data model">
---
...
2. Read the relevant skills for the area being audited:
   - Components/pages → `frontend-patterns`
   - CSS/Tailwind → `frontend-patterns`
   - Docker/build → `dev-workflow`
...
|---|------|-------|--------------------|-----------|
| 1 | `src/components/WorkCard.astro` | Hardcoded content | Data must live in `src/data/` | Move to JSON |
| 2 | ... | ... | ... | ... |
...
If the build fails: diagnose and fix before handing off.
