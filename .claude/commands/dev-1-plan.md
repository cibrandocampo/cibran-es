...
4. Ask the user everything needed with `AskUserQuestion`:
   ...
   - What stack layers does it affect? (components, data, styles, infra)
   - Are there open design decisions?
   - Are there constraints or preferences? (e.g.: "no JS", "static only", "content only")
...
2. Identify:
   ...
   - Existing patterns that must be followed (consult `frontend-patterns` skill).
