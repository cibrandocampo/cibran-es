...
1. Identify the affected layers (components, data, styles, infra).
2. Group changes by layer — each layer is usually 1 task.
3. Build verification is always the last step of each task.
...
1. Build passes (`docker compose -f docker-compose.dev.yml run --rm web npm run build`)
2. ...
...
| File | Action |
|------|--------|
| `src/components/SomeComponent.astro` | CREATE |
| `src/data/some-data.json` | MODIFY |
```
...
- **Docker commands**: always with `docker compose -f docker-compose.dev.yml run --rm web`.
